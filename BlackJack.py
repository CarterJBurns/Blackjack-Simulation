...
def play_of_hand(first_card: int, second_card: int, dealer_up_card: int, player_total: int, bet_amount: int) -> str:
    global play_of_hand_count, total_money
...
def automation(initial_bet_amount: int, method: str) -> None:
    global blackjackcount, win_loss_history, total_money, total_money_list, play_of_hand_count
    ...
    if player_total == 21:
        ...
        result = play_of_hand(first_card, second_card, dealer_up_card, player_total, bet_amount)
...
def helpmenu() -> None:
    print("\nMartingale System:")
    print("Start with an initial bet. \nIf you lose, double your bet. \nIf you win, return to the initial bet.\n")

    print("Reverse Martingale System:")
    print("Start with an initial bet.\nIf you win, double your bet.\nIf you lose, return to the initial bet.\n")

    print("Paroli")
    print("Start with an initial bet.\nIf you win, double your bet for the next round.\nIf you lose, return to the initial bet.\nAfter three consecutive wins, return to the initial bet.\n")

    print("Flat Betting")
    print("Same bet amount every time.\n")
...
def main(strategy = "H") -> None:
    global blackjackcount, win_loss_history, total_money, bet_amount, method, total_money_list, play_of_hand_count

    total_money = 10000
    blackjackcount = 0
    bet_amount = 10
    win_loss_history = []
    total_money_list = [10000]
    play_of_hand_count = 0
    method = strategy
    printmenu()
    #method = input("Which method would you like to use ((M)artingale, (R)everse Martingale, (P)aroli, (F)lat, or (H)elp for more information): ").upper()
    """
    while method == 'H' or method not in ("M", "R", "P", "F"):
        helpmenu()
        method = input("Which method would you like to use ((M)artingale, (R)everse Martingale, (P)aroli, (F)lat, or (H)elp for more information): ").upper()
    """
    while total_money > 0 and total_money >= bet_amount:
        automation(bet_amount, method)
    ...
    #print(f'Max Winstreak: {max_winstreak}')
    #print(f'Max Lostreak: {max_losstreak}')
